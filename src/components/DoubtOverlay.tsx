    ...
    <>
      ...
      >
        ...
        <div className="relative flex flex-col items-center px-6 pt-6 pb-7 text-center">
          ...
          <div className="text-[2rem] mb-3 text-accent/70" style={visible ? { animation: "doubt-core-breathe 3s ease-in-out infinite" } : undefined}>✦</div>
          <p className="font-[var(--font-cinzel-decorative)] text-[1rem] text-accent/70 mb-1.5 tracking-[0.15em] uppercase" style={visible ? { animation: "doubt-text-in 0.6s ease-out 0.1s both" } : undefined}>
            Le grimoire hésite
          </p>
          <div className="flex items-center justify-center gap-2.5 mb-3">
            <div className="h-[1px] w-8 bg-gradient-to-r from-transparent to-accent/30 origin-right" style={visible ? { animation: "doubt-line-grow 0.5s ease-out 0.3s both" } : undefined} />
            <span className="text-[0.35rem] text-accent/25">◆</span>
            <div className="h-[1px] w-8 bg-gradient-to-l from-transparent to-accent/30 origin-left" style={visible ? { animation: "doubt-line-grow 0.5s ease-out 0.3s both" } : undefined} />
          </div>
          <p className="font-[var(--font-cinzel)] text-[0.90rem] text-text/80 leading-relaxed" style={visible ? { animation: "doubt-text-in 0.6s ease-out 0.25s both" } : undefined}>
            Es-tu certaine de vouloir soumettre<br />
            <span className="text-accent/60 text-[0.85rem]">cette réponse aux runes ?</span>
          </p>
        </div>

        <div className="relative px-5 pb-8 pt-3 flex gap-3" style={visible ? { animation: "doubt-btn-in 0.5s ease-out 0.45s both" } : undefined}>
          <button
            onClick={onCancel}
          ...
          <button
            ...
            style={{ background: "linear-gradient(160deg, #5a3a8a, #7b5dbd)", boxShadow: "0 4px 20px #9b6dff25, inset 0 1px 0 #ffffff10" }}
          >
            <div className="absolute inset-0 pointer-events-none opacity-30" style={{ background: "linear-gradient(90deg, transparent 0%, rgba(255,255,255,0.3) 50%, transparent 100%)", backgroundSize: "200% 100%", animation: "envelope-shimmer 3s ease-in-out infinite" }} />
            <span className="relative z-[1] flex items-center justify-center gap-1.5">
              J'en suis sûre
