...
function ModalBody({
  ...
  function resolve() {
    if (normalize(value) === normalize(enigma.answer)) {
      ...
      sndOk();
      recordAttempt();
      setTimeout(() => { closeModal(); celebrate(enigma.id); }, SOLVE_FEEDBACK_MS);
    } else {
      setFeedback("err");
    ...
    >
      ...
      {!isSolved && attemptUsed && (
        <div className="text-center py-5">
          ...
          <p className="text-[1.4rem] font-[var(--font-cinzel)] text-accent tracking-[0.15em] mt-1.5">{countdown}</p>
        </div>
      )}
...
export function EnigmaModal() {
  ...
  return (
    ...
    >
      {enigma && (
        <ModalBody key={displayId} enigma={enigma} isSolved={isSolved} attemptUsed={attemptUsed} isOpen={isOpen} />
      )}
    </div>
  );
}
