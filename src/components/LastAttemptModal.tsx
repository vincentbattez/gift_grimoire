...
export function LastAttemptModal({ onConfirm }: { onConfirm: () => void }) {
  ...
  return (
    ...
    >
      <div
        className={`w-full max-w-[360px] mx-4 flex flex-col items-center text-center py-8 px-6 rounded-[18px] border border-danger/25 transition-all duration-300 ${
      ...
      >
        <span
          ...
          style={{ filter: "drop-shadow(0 0 16px #ff6b8a80)" }}
        >
          ✦⏳
        </span>
        ...
        <p className="text-[0.82rem] leading-relaxed text-text/80 whitespace-pre-line max-w-[300px] mb-4">
        ...
        <button
          ...
          className="py-3 px-8 rounded-full border border-danger/40 bg-danger/10 text-danger text-[0.78rem] font-[var(--font-cinzel)] tracking-[0.15em] uppercase transition-all duration-300 hover:bg-danger/20 active:scale-95 cursor-pointer"
        >
          Je suis prêt ✦
        </button>
      </div>
