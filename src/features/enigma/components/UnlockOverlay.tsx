...
import { triggerUnlockReveal } from "@features/enigma/unlock";
...
export function UnlockOverlay(): React.JSX.Element | null {
  ...
  const getDistToKeyhole = useCallback(
    (px: number, py: number) => {
      const keyholeCenter = getKeyholeCenter();

      if (!keyholeCenter) {
        return Infinity;
      }

      return Math.hypot(px - keyholeCenter.x, py - keyholeCenter.y);
    },
    [getKeyholeCenter],
  );
  ...
  const onDragStart = useCallback(
    (clientX: number, clientY: number, elRect: DOMRect) => {
      if (phase !== "drag") {
        return;
      }
  ...
  const onDragMove = useCallback(
    (clientX: number, clientY: number) => {
      ...
      const distanceToKeyhole = getDistToKeyhole(x, y);
      setProximity(Math.max(0, Math.min(1, 1 - distanceToKeyhole / 200)));
    },
  ...
  const triggerUnlockSequence = useCallback(
    (keyholeCenter: { x: number; y: number }) => {
      ...
      const t2 = setTimeout(() => {
        ...
        setTimeout(() => {
          spawnParticles(keyholeCenter.x, keyholeCenter.y, 40, "#e8c96a");
        }, 150);
  ...
  const onDragEnd = useCallback(
    (clientX: number, clientY: number) => {
      ...
      const distanceToKeyhole = getDistToKeyhole(clientX - dragOffset.current.x, clientY - dragOffset.current.y);

      if (distanceToKeyhole < SNAP_THRESHOLD) {
        ...
        if (keyholeCenter) {
          setKeyPos({ x: keyholeCenter.x, y: keyholeCenter.y - 50 });
          setPhase("unlocking");
          triggerUnlockSequence(keyholeCenter);
        }
      } else {
        // Spring back
        setKeyPos(null);
  ...
  const onTouchStart = useCallback(
    (e: React.TouchEvent<HTMLDivElement>) => {
      ...
      const rect = e.currentTarget.getBoundingClientRect();
      onDragStart(touch.clientX, touch.clientY, rect);
    },
    [onDragStart],
  );
  ...
  const restY = window.innerHeight - 120;
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          transform: isSceneVisible ? "scale(1)" : "scale(1.2)",
          filter: isSceneVisible ? "none" : "blur(6px)",
        ...
        {isDone &&
          Array.from({ length: 8 }).map((_, i) => (
            <div
              ...
              style={
                {
                  "--frag-angle": `${String((360 / 8) * i)}deg`,
                  "--frag-dist": `${String(FRAG_DISTANCE_LIST[i])}px`,
      ...
      {/* Hint text */}
      {phase === "drag" && <div className="unlock-hint">Glisse la clé dans la serrure</div>}

      {/* Enigma reveal after unlock */}
      {isDone &&
        (() => {
          const enigma = ENIGMA_LIST.find((e) => e.id === unlockingId);

          return (
            <div className="unlock-reveal is-visible">
              <div className="unlock-reveal-label">Énigme {enigma?.id}</div>
              <div className="unlock-reveal-icon">{enigma?.icon}</div>
              <div className="unlock-reveal-title">{unlockingTitle}</div>
            </div>
          );
        })()}
      ...
      {!isDone && (
        <div
          ...
          style={{
            ...
            filter: `drop-shadow(0 0 ${String(8 + proximity * 20)}px rgba(232, 201, 106, ${String(0.4 + proximity * 0.6)}))`,
          }}
